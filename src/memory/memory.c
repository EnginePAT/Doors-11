...
void memMapPage(uint32_t virutalAddr, uint32_t physAddr, uint32_t flags){
    ...
    if (!(pageDir[pdIndex] & PAGE_FLAG_PRESENT)){
        ...
        pageDir[pdIndex] = ptPAddr | PAGE_FLAG_PRESENT | PAGE_FLAG_WRITE | PAGE_FLAG_OWNER;
        invalidate(virutalAddr);

        uint32_t* pt = REC_PAGETABLE(pdIndex); // re-read after allocation
...
bool initMemory(uint32_t memHigh, uint32_t physicalAllocStart){
    mem_num_vpages = 0;
    ...
    // --- Map a fixed heap upfront ---
    uint32_t heap_base = KERNEL_MALLOC;        // e.g. 0x01000000
    uint32_t heap_size = 32 * 1024 * 1024;     // 32 MB

    for (uint32_t offset = 0; offset < heap_size; offset += 0x1000) {
        uint32_t phys = pmmAllocPageFrame();
        if (!phys) {
            printf("pmmAllocPageFrame ran out while mapping heap!\n");
            return false;
        }
        memMapPage(heap_base + offset, phys, PAGE_FLAG_WRITE);
    }
